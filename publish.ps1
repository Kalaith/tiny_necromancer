...
$workspaceParent = Split-Path $PSScriptRoot -Parent
$rootPublisher = Join-Path $workspaceParent "publish.ps1"

if (-not (Test-Path $rootPublisher)) {
    $rootPublisher = Join-Path (Join-Path $workspaceParent "rust_management") "publish.ps1"
}
