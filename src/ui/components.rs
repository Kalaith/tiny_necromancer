...
pub fn pause_control_rect() -> Rect {
    Rect::new(1172.0, 19.0, 80.0, 44.0)
}

pub fn placement_cancel_rect() -> Rect {
...
pub(super) fn draw_phase_overlay(
    ...
    match ctx.session.phase {
        GamePhase::MainMenu => draw_main_menu(ctx, pointer, actions),
        GamePhase::Paused => draw_paused_overlay(ctx, pointer, actions),
        GamePhase::Victory => draw_victory_overlay(pointer, actions),
        GamePhase::Error => draw_error_overlay(ctx, pointer, actions),
        GamePhase::Playing => {}
    }
}

fn draw_main_menu(ctx: &UiContext<'_>, pointer: Pointer, actions: &mut Vec<UiAction>) {
    full_screen_overlay(0.34);
    draw_surface(
        Rect::new(360.0, 150.0, 560.0, 420.0),
        &SurfaceStyle::new(Color::new(0.055, 0.055, 0.065, 0.96)).with_border(2.0, dark::ACCENT),
    );
    draw_text_centered_in_box(
...
fn draw_paused_overlay(ctx: &UiContext<'_>, pointer: Pointer, actions: &mut Vec<UiAction>) {
    full_screen_overlay(0.44);
    draw_surface(
        Rect::new(420.0, 250.0, 440.0, 240.0),
        &SurfaceStyle::new(Color::new(0.06, 0.06, 0.07, 0.98)).with_border(2.0, dark::ACCENT),
    );
...
fn draw_victory_overlay(pointer: Pointer, actions: &mut Vec<UiAction>) {
    full_screen_overlay(0.38);
    draw_surface(
        Rect::new(360.0, 190.0, 560.0, 330.0),
        &SurfaceStyle::new(Color::new(0.055, 0.065, 0.065, 0.99)).with_border(2.0, dark::POSITIVE),
    );
    draw_text_centered_in_box(
    ...
    draw_text_block(
        "The cemetery now has workers, shelter, light, and room to grow. The road still has not found you.",
        420.0,
        302.0,
        440.0,
        60.0,
...
fn draw_error_overlay(ctx: &UiContext<'_>, pointer: Pointer, actions: &mut Vec<UiAction>) {
    full_screen_overlay(0.55);
    draw_surface(
        Rect::new(360.0, 230.0, 560.0, 240.0),
        &SurfaceStyle::new(Color::new(0.15, 0.07, 0.08, 0.99)).with_border(2.0, dark::NEGATIVE),
    );
    draw_text_centered_in_box(
