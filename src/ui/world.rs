...
pub(super) fn draw_world_scene(ctx: &UiContext<'_>) {
    ...
    structures::draw_graves(ctx, &view);
    structures::draw_buildings(ctx, &view);
...
fn draw_path_and_props(ctx: &UiContext<'_>, view: &GridView) {
    ...
    let stock = view
        .tile_rect(ctx.session.world.stockpile_position())
        .inset(10.0);
    draw_rectangle(
        stock.x,
...
fn draw_placement_preview(ctx: &UiContext<'_>, view: &GridView, kind: BuildingKind) {
    let mouse = ctx.ui.mouse_position();
    let tile = selected_tile_at(ctx, mouse).unwrap_or(ctx.session.world.stockpile_position());
...
fn placement_valid(
    ...
    if progression::placement_requires_gravecraft(ctx.session, ctx.data, kind, position) {
