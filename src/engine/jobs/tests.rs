...
#[test]
fn secure_policy_staffs_an_uncovered_patrol_post_at_calm() {
    let data = crate::data::GameData::load().unwrap();
    let mut session = GameSession::new(&data.config);
    ...
    session.world.zones.push(crate::state::Zone {
        ...
        tiles: vec![macroquad_toolkit::grid::TilePos::new(5, 1)],
    });
    session.workforce.workers[0].priority_mode = true;
    session.workforce.workers[0].assignment = JobKind::Dig;
...
fn patrol_target_prefers_a_reachable_marked_post() {
    let data = crate::data::GameData::load().unwrap();
    let mut session = GameSession::new(&data.config);
    let reachable = macroquad_toolkit::grid::TilePos::new(1, 4);
...
#[test]
fn builder_skips_a_sealed_first_scaffold() {
    let data = crate::data::GameData::load().unwrap();
    let mut session = GameSession::new(&data.config);
    let reachable = macroquad_toolkit::grid::TilePos::new(1, 4);
    session.world.buildings = vec![
        crate::state::Building {
            ...
            progress: 2.0,
            complete: false,
            position: macroquad_toolkit::grid::TilePos::new(7, 1),
            width: 1,
            height: 1,
        },
        crate::state::Building {
            kind: crate::state::BuildingKind::GraveLantern,
            progress: 2.0,
            complete: false,
            position: macroquad_toolkit::grid::TilePos::new(2, 4),
            width: 1,
            height: 1,
        },
    ];
    session.workforce.workers[0].assignment = JobKind::Build;
    for y in 0..session.world.height as i32 {
        session.world.buildings.push(crate::state::Building {
            kind: crate::state::BuildingKind::WorkShed,
            progress: 10.0,
