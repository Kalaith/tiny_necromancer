...
use tiny_necromancer::state::{GameSession, JobKind, RoutePolicy, Technology};
#[test]
fn hauler_commits_the_source_and_marked_storage_drop_when_loading() {
    ...
    assert_eq!(plan.resource, tiny_necromancer::state::ResourceKind::Bones);
...
fn changed_storage_policy_replans_a_carried_bundle_without_dropping_it() {
    ...
    assert_eq!(plan.destination, session.world.stockpile_position());
...
fn empty_reserved_source_releases_a_hauler_to_the_next_pile() {
    ...
    session.economy.add_loose(
        tiny_necromancer::state::ResourceKind::Bones,
        first_source,
        8,
    );
    session.economy.add_loose(
    ...
    let mut second_worker = session.workforce.workers[0].clone();
    second_worker.id = session.workforce.next_worker_id;
    second_worker.haul_plan = Some(tiny_necromancer::state::HaulPlan {
        resource: tiny_necromancer::state::ResourceKind::Bones,
        source: first_source,
        destination: session.world.stockpile_position(),
        storage_policy: RoutePolicy::MarkedFirst,
        destination_kind: tiny_necromancer::state::HaulDestination::Storage,
    });
    session.workforce.next_worker_id += 1;
    session.workforce.workers.push(second_worker);
