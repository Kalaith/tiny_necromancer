...
5. **First colony slice (implemented for the current slice).** The Ossuary Kiln completes a material → construction → production chain after Ossuary Logistics, workers route around basic obstructions before working, and painted Work, Storage, and Patrol areas select nearby destinations. Loose output stays separated by source pile, while haulers commit a source-to-drop route, deliver missing recipe inputs directly to the kiln, and replan a carried bundle when the Storage policy or route changes. The minimap and worker/grave cues provide first-pass domain feedback while keeping the original cemetery recognizable as part of the expanded settlement.
