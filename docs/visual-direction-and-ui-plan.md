...
| `src/ui/world.rs`, `src/ui/world_feedback.rs`, and `src/ui/animation.rs`: full-world grid, actor interpolation, animation, complete route previews, and source feedback | Deeper district simulation and multi-district route policies |
...
| `src/engine/progression.rs`, `src/engine/alerts.rs`, `src/engine/districts.rs`: research-driven capabilities, milestone, blockers, and district rules | Extend stewardship policies beyond district-aware worker priorities |
