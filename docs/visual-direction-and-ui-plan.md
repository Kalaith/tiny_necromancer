...
| `src/ui/world.rs`, `src/ui/world_feedback.rs`, and `src/ui/animation.rs`: full-world grid, actor interpolation, animation, complete route previews, and source feedback | Deeper economy and multi-district route policies |
...
| `src/state.rs`: positioned buildings, research, zones, movement destinations, carried resources, and save migration | Extend stewardship policies beyond district-aware worker priorities |
| `src/engine/progression.rs`, `src/engine/alerts.rs`, `src/engine/districts.rs`: research-driven capabilities, milestone, blockers, district rules, and route-aware service coverage | Extend stewardship policies beyond district-aware worker priorities |
