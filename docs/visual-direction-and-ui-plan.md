...
| Building | Construction state or output, staff, requirements | Assign, queue or cancel reserved production, reinforce when supported |
...
The current game has a 10 × 8 world, six authored grave positions, one starting skeleton, three building types, jobs, repeat priorities, corpse qualities, suspicion events, versioned save support, a selectable necromancer, positioned buildings, four research technologies, editable work areas, deterministic worker routing, per-source loose-resource piles, finite material storage with marked-district expansion, route-aware Work/Storage/Patrol destinations, roster-aware marked Storage slot routing, stable multi-post patrol coverage, reachable patrol coverage accounting, aggregated storage and patrol districts, an Ossuary Kiln production recipe with a three-cycle queue, a district overview with a stateful minimap, actionable operational alerts with a seven-entry History drawer, Domain Stewardship overlays for zones, worker routes, and road pressure, persistent Balanced, Secure, and Harvest policies for automated workers, policy-aware Harvest district prioritization after Domain Stewardship, route-aware priority recovery for automated workers, location-aware Work/Storage/Patrol district rules that affect workers operating on marked tiles after Domain Stewardship, capacity-aware district staffing demand, actionable Work and Storage district staffing advisories, route-gap advisories that select an assigned worker when district paths are blocked, a marked-tile district inspector, a save-persistent district performance ledger with first-use Field Notes and a bounded recent activity trail, a Domain staffing summary for Work, Storage, and Patrol, route-aware district service coverage that separates assigned hands from reachable hands, explicit source-to-Storage haul plans that replan safely when policy or routes change, one-tier persistent building reinforcements with structure-specific effects, and touch camera pan/pinch with visible zoom and recenter controls on compact viewports. The sprite sheet is registered through the texture manifest and falls back to an obvious placeholder when unavailable. The current victory condition ends the slice after the small cemetery is established; deeper economy remains future work.
...
| `src/ui/world.rs`, `src/ui/world_feedback.rs`, and `src/ui/animation.rs`: full-world grid, actor interpolation, animation, complete route previews, and source feedback | Deeper economy, trade, and multi-resource chains |
...
| `src/state.rs`, `src/state/economy.rs`, and `src/state/building_upgrades.rs`: positioned buildings, research, zones, movement destinations, per-source loose piles, carried resources, finite storage, haul plans, one-tier building reinforcements, and save migration | Add multi-resource chains |
| `src/engine/progression.rs`, `src/engine/alerts.rs`, `src/engine/districts.rs`: research-driven capabilities, milestone, blockers, district rules, and route-aware service coverage | Add deeper production and trade pressure |
...
2. **One convincing cemetery scene (implemented for the slice).** Terrain and sprites cover the necromancer, skeleton, graves, trees, stockpile, shed, and lantern. Digging, hauling, construction progress, selection, camera transforms, route intent, per-source loose piles, and source feedback are readable.
...
5. **First colony slice (implemented for the current slice).** The Ossuary Kiln completes a material → construction → production chain after Ossuary Logistics, workers route around basic obstructions before working, and painted Work, Storage, and Patrol areas select nearby destinations. Loose output stays separated by source pile, while haulers commit a source-to-drop route, deliver missing recipe inputs directly to the kiln (including directly from a loose pile when storage is empty), and replan a carried bundle when the Storage policy or route changes. The minimap and worker/grave cues provide first-pass domain feedback while keeping the original cemetery recognizable as part of the expanded settlement.
6. **Scale and domain controls (implemented for the current slice).** The settlement overview includes useful operational alerts with source selection, Domain Stewardship controls for district marks, worker routes, and road-pressure coverage, actionable Work and Storage district staffing guidance, marked-tile rule inspection, persistent Balanced, Secure, and Harvest policies that affect automated worker choices, Harvest fill marked district gaps after Domain Stewardship, capacity-aware staffing slots that remain visible until concurrent district demand is met, route-aware service coverage that separates assignment gaps from blocked paths, roster-aware Storage drop-point routing across concurrent marked slots, source-to-drop haul plans that stay stable across route changes and replan safely when policy changes, reachable-duty recovery for automated workers when a priority destination is sealed, selected-worker fallback feedback that names the skipped priority and notices a duty change, save-persistent Marked first, Nearest, and Marked only route policies for repeat Work, Storage, and Patrol duties, additive Work, Storage, and Patrol rules that make marked districts operationally meaningful, a save-persistent performance ledger with a bounded activity trail that records their real contribution, and a compact staffing summary that compares district operators with marked Patrol posts. Future work can add deeper economy while validating worker selection and performance before growing the content set.
